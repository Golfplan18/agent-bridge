...
`claude`. Three more identifiers are real but have no connector in this build —
`zcode`, `hermes` and `minimax-code` — and asking for one of them fails and says
so. If the user did not name a peer, ask which one; do not pick for them.
