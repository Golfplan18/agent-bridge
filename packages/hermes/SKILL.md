...
**1. Choose the peer.** Two peers can be called: `codex` and Claude Code's own
`claude`. Three more identifiers are real but cannot be called in this build —
`zcode`, `hermes` and `minimax-code` — and asking for one of them fails and
says why. If the user did not name a peer, ask which one; do not pick for them.
...
Ask about the two peers that can be called, one at a time:
