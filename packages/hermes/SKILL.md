...
`claude`, and Hermes Agent itself as `hermes`. Two more identifiers are real
but cannot be called in this build — `zcode` and `minimax-code` — and asking
for one of them fails and says so. If the user did not name a peer, ask which
