---
name: agent-bridge
description: Carry one Markdown message to Codex, Claude Code, ZCode, or Hermes Agent through Agent Bridge, report target readiness, or add a neutral session note. Use when the user asks ZCode to consult a supported coding-agent harness or inspect an existing Agent Bridge session.
...
Hermes is courier-only. A Hermes session cannot have a project, so include any
needed source material in the outgoing body.
...
python3 -m bridge record --session <absolute-session-directory> --kind session-create --initiator zcode --peer <codex|claude|zcode|hermes> [--project <absolute-project-directory>] < /absolute/path/to/session-body.md
python3 -m bridge run --session <absolute-session-directory> [--timeout <seconds>] < /absolute/path/to/outgoing-body.md
python3 -m bridge record --session <absolute-session-directory> --kind note < /absolute/path/to/note-body.md
```
