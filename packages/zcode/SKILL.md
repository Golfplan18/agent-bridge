...
when_to_use: The user wants another coding agent's view on something, names Codex or Claude Code as someone to ask, wants to continue a conversation already held in an Agent Bridge session folder, or asks which bridge peers are ready to be called.
license: Unlicense
---
...
This skill runs inside ZCode. Every command below is run with ZCode's own shell
tool, one command at a time, exactly as written.
...
ZCode lists each skill with the path of its `SKILL.md`; if you know the
directory this file is in, pass that directory. If you do not have it, pass an
empty string instead and that one step is skipped.
...
copy under `~/.zcode/skills/agent-bridge/`, and that is the right answer), the
...
**1. Choose the peer.** Two peers can be called from here: `codex` and Claude
Code's own `claude`. ZCode's own identifier, `zcode`, is you — do not ask it as
a peer. Two more identifiers are real but have no connector in this build —
`hermes` and `minimax-code` — and asking for one of them fails and says so. If
the user did not name a peer, ask which one; do not pick for them.
...
`--local zcode` is you: this skill runs inside ZCode. `--workflow` accepts
...
later ZCode session that knows nothing of this one — can pick the conversation
up by reading them.
...
Ask about the two peers that can be called from here, one at a time:
...
```text
python3 -m bridge check --peer <codex|claude|zcode|hermes|minimax-code>
...
## Installing and removing this skill

ZCode discovers a user-scope skill at `~/.zcode/skills/<name>/SKILL.md`. To
install, copy this one file to `~/.zcode/skills/agent-bridge/SKILL.md`; to
remove it, delete that one directory. Nothing else is written anywhere, and
sessions under `~/.agent-bridge/sessions/` are untouched either way.
